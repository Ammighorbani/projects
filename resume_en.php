...
// اطلاعات شخصی
$stmt = $conn->prepare("SELECT linkedin, github, personal_stmt FROM personal_information_en WHERE user_id=?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
...
// خلاصه حرفه‌ای
$stmt = $conn->prepare("SELECT summary FROM professional_summary_en WHERE user_id=?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
...
// پاسپورت
$stmt = $conn->prepare("SELECT passport_number FROM passport_info_en WHERE user_id=?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$row = $stmt->get_result()->fetch_assoc();
...
// ویزا
$stmt = $conn->prepare("SELECT description FROM visa_info_en WHERE user_id=?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$row = $stmt->get_result()->fetch_assoc();
...
// تجربه کاری
$stmt = $conn->prepare("SELECT *, start_date AS start, end_date AS end FROM work_experience_en WHERE user_id=? ORDER BY job_index");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
// تحصیلات
$stmt = $conn->prepare("SELECT * FROM education_en WHERE user_id=? ORDER BY education_index");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
// مهارت‌ها
$stmt = $conn->prepare("SELECT * FROM skills_en WHERE user_id=? ORDER BY skill_index");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
// دوره‌ها
$stmt = $conn->prepare("SELECT *, received_date AS date FROM courses_certificates_en WHERE user_id=? ORDER BY course_index");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
// پروژه‌ها
$stmt = $conn->prepare("SELECT * FROM project_en WHERE user_id=? ORDER BY project_index");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$res = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO personal_information_en 
    ...
    // خلاصه + پاسپورت + ویزا
    $stmt = $conn->prepare("INSERT INTO professional_summary_en (user_id, summary) VALUES (?, ?) ON DUPLICATE KEY UPDATE summary=?");
    $stmt->bind_param("iss", $user_id, $_POST['summary'], $_POST['summary']);
    $stmt->execute();

    $stmt = $conn->prepare("INSERT INTO passport_info_en (user_id, passport_number) VALUES (?, ?) ON DUPLICATE KEY UPDATE passport_number=?");
    $stmt->bind_param("iss", $user_id, $_POST['passport'], $_POST['passport']);
    $stmt->execute();

    $stmt = $conn->prepare("INSERT INTO visa_info_en (user_id, description) VALUES (?, ?) ON DUPLICATE KEY UPDATE description=?");
    ...
    saveList('work',      'work_experience_en',       'job_index',       ['job_title','company_name','company_city_province'], ['start_date','end_date','bullet1','bullet2','bullet3','bullet4','bullet5']);
    saveList('education', 'education_en',             'education_index', ['university_name','field_of_study','graduation_year','gpa','projects','honors'], []);
    saveList('skills',    'skills_en',                'skill_index',     ['skill_name','skill_type','skill_level'], []);
    saveList('courses',   'courses_certificates_en',  'course_index',    ['course_name','received_date'], []);
    saveList('projects',  'project_en',               'project_index',   ['type','title','description'], []);
    saveList('languages', 'languages_en', 'language_index', ['language_name','proficiency'], []);
...
<div class="c">
    ...
    <form method="post">
        <div class="s"><h2>Personal information</h2>
        ...
        <div class="s"><h2>Professional Summary</h2>
            <textarea name="summary" placeholder="خلاصه حرفه‌ای"><?=htmlspecialchars($summary)?></textarea>
        </div>

        <div class="s"><h2>Visa & Passport Information</h2>
        ...
        <div class="s"><h2>Work Experiences<button type="button" class="add" onclick="addWork()">+ اضافه کردن</button></h2>
        ...
        <div class="s"><h2>Educations<button type="button" class="add" onclick="addEdu()">+ اضافه کردن</button></h2>
            <div id="eduContainer">
        ...
        <div class="s"><h2>Skills<button type="button" class="add" onclick="addSkill()">+ اضافه کردن</button></h2>
        ...
        <div class="s"><h2>Courses & Certificates<button type="button" class="add" onclick="addCourse()">+ اضافه کردن</button></h2>
            <div id="courseContainer">
                ...
                <!-- === بخش زبان‌ها === -->
        <div class="s">
            <h2>Languages<button type="button" class="add" onclick="addLanguage()">+ اضافه کردن</button></h2>
            <div id="languageContainer">
                <?php 
                $stmt = $conn->prepare("SELECT * FROM languages_en WHERE user_id=? ORDER BY language_index");
                $stmt->bind_param("i", $user_id);
                $stmt->execute();
                $res = $stmt->get_result();
                $langCount = 0;
        ...
        <div class="s"><h2>Projects<button type="button" class="add" onclick="addProject()">+ اضافه کردن</button></h2>
        ...
        <button type="submit" onclick="checkPaymentAndSubmit()">Save Resume</button>
    </form>
</div>
    ...
    let html = `
        <div class="item">
            ...
            <input type="text" 
                   ...
                   placeholder="Language Name: English, Persian, French..." 
                   oninput="this.value = this.value ? this.value.charAt(0).toUpperCase() + this.value.slice(1).toLowerCase() : ''"
                   style="width: 48%; margin-left: 4px;">
