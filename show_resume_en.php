...
// مهارت‌ها
$stmt = $conn->prepare("SELECT * FROM skills_en WHERE user_id=? AND skill_name != '' ORDER BY skill_index");
$stmt->bind_param("i", $user_id); $stmt->execute(); $res = $stmt->get_result();
$skills = $res->fetch_all(MYSQLI_ASSOC);
$stmt->close();

// دوره‌ها
$stmt = $conn->prepare("SELECT *, received_date AS date FROM courses_certificates_en WHERE user_id=? AND course_name != '' ORDER BY course_index");
$stmt->bind_param("i", $user_id); $stmt->execute(); $res = $stmt->get_result();
$courses = $res->fetch_all(MYSQLI_ASSOC);
$stmt->close();

// پروژه‌ها
$stmt = $conn->prepare("SELECT * FROM project_en WHERE user_id=? AND title != '' ORDER BY project_index");
$stmt->bind_param("i", $user_id); $stmt->execute(); $res = $stmt->get_result();
$projects = $res->fetch_all(MYSQLI_ASSOC);
$stmt->close();

// زبان‌ها
$stmt = $conn->prepare("SELECT language_name, proficiency FROM languages_en WHERE user_id=? ORDER BY language_index");
$stmt->bind_param("i", $user_id); $stmt->execute(); $res = $stmt->get_result();
